...
date; grep blackridge /var/log/messages |egrep -v systemd-logind\|dbus\|\ su: |sed 's/Password:..*$/Password: XXXX/' > /var/www/html/honey/blackridge/current-raw-data.txt
date; grep blackridge /var/log/messages |egrep -v systemd-logind\|dbus\|\ su: |sed 's/Password:..*$/Password: XXXX/'|grep 'blackridge sshd\[' > /var/www/html/honey/blackridge/current-identified-traffic.txt
ONE_AM=1
